#!/usr/bin/env python3
"""
report4_streaming.py - Structured Streaming for Report 4 only: Record count + revenue per 15-min window by paytype

This script processes streaming CSV files and generates 15-minute record count and revenue aggregations
...
from pyspark.sql.functions import col, to_timestamp, sum as _sum, count as _count, window
...
def write_count_revenue_15min_to_csv(df, epoch_id):
    """Custom function to write 15-min record count + revenue by paytype to CSV"""
    try:
        if df.count() > 0:
            print(f"Processing 15-min count/revenue epoch {epoch_id} with {df.count()} rows")
            
            # Write to a single location (overwrites previous with complete results)
            output_path = f"{OUTPUT_BASE}/count_revenue_15min"
            df.coalesce(1).write \
                .mode("overwrite") \
                .option("header", True) \
                .csv(output_path)
            ...
            print(f"15-min count/revenue data for epoch {epoch_id} (showing first 10 rows):")
            df.show(10, truncate=False)
            
            print(f"✓ Successfully written 15-min count/revenue CSV for epoch {epoch_id} to {output_path}")
        else:
            print(f"No 15-min count/revenue data in epoch {epoch_id}")
    except Exception as e:
        print(f"❌ Error in 15-min count/revenue epoch {epoch_id}: {str(e)}")
        import traceback
        traceback.print_exc()

# Spark session with streaming configs
builder = SparkSession.builder.appName("Report4_15min_CountRevenue_Streaming") \
...
print("="*60)
print("REPORT 4: 15-MIN RECORD COUNT + REVENUE BY PAYTYPE - STREAMING")
print("="*60)
print(f"Reading from: {INPUT_DIR}")
print(f"Output to: {OUTPUT_BASE}/count_revenue_15min")
print(f"Checkpoint: {CHECKPOINT_BASE}/count_revenue_15min")
...
print(f"Reference map has {ref_df.count()} paytype mappings")
...
# Parse & clean (same logic as batch)
...
# Join with static ref map
streaming_df = streaming_df.join(ref_df, on="paytype", how="left")
...
print("Creating 15-minute count/revenue aggregation...")
count_revenue_15min = streaming_df_watermarked.groupBy(
...
count_query = count_flat.writeStream \
    .outputMode("complete") \
    .foreachBatch(write_count_revenue_15min_to_csv) \
    .trigger(processingTime=TRIGGER_INTERVAL) \
    .option("checkpointLocation", f"{CHECKPOINT_BASE}/count_revenue_15min") \
    .queryName("count_revenue_15min_stream") \
    .start()

print("✓ Streaming query started successfully!")
print(f"Query ID: {count_query.id}")
print(f"Query Name: {count_query.name}")
print("\nWaiting for streaming data...")
print("Press Ctrl+C to stop the application\n")

try:
    # Monitor the streaming query status
    while count_query.isActive:
        status = count_query.status
        print(f"Query Status: {status['message'] if 'message' in status else 'Running'}")
        
        # Show recent progress
        progress = count_query.lastProgress
        if progress:
        ...
        exception = count_query.exception()
        if exception:
    ...
    if count_query.isActive:
        count_query.stop()
    spark.stop()
    print("✅ Report 4 streaming application stopped successfully.")
    print(f"Final output location: {OUTPUT_BASE}/count_revenue_15min")
