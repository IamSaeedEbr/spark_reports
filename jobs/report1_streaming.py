#!/usr/bin/env python3
"""
report1_streaming.py - Structured Streaming for Report 1 only: Daily revenue

This script processes streaming CSV files and generates daily revenue aggregations,
saving results as CSV files to MinIO/S3.
"""
...
from pyspark.sql.functions import col, to_timestamp, date_format, sum as _sum
...
def write_daily_to_csv(df, epoch_id):
    """Custom function to write daily revenue to CSV"""
    try:
        if df.count() > 0:
            print(f"Processing daily revenue epoch {epoch_id} with {df.count()} rows")
            
            # Write to a single location (overwrites previous with complete results)
            output_path = f"{OUTPUT_BASE}/daily_revenue"
            df.coalesce(1).write \
                .mode("overwrite") \
                .option("header", True) \
                .csv(output_path)
            ...
            print(f"Daily revenue data for epoch {epoch_id}:")
            df.show(10, truncate=False)
            
            print(f"✓ Successfully written daily revenue CSV for epoch {epoch_id} to {output_path}")
        else:
            print(f"No daily revenue data in epoch {epoch_id}")
    except Exception as e:
        print(f"❌ Error in daily revenue epoch {epoch_id}: {str(e)}")
        import traceback
        traceback.print_exc()

# Spark session with streaming configs
builder = SparkSession.builder.appName("Report1_Daily_Revenue_Streaming") \
...
print("="*60)
print("REPORT 1: DAILY REVENUE - STREAMING")
print("="*60)
print(f"Reading from: {INPUT_DIR}")
print(f"Output to: {OUTPUT_BASE}/daily_revenue")
print(f"Checkpoint: {CHECKPOINT_BASE}/daily_revenue")
...
daily_revenue = streaming_df_watermarked \
...
daily_query = daily_revenue.writeStream \
    .outputMode("complete") \
    .foreachBatch(write_daily_to_csv) \
    .trigger(processingTime=TRIGGER_INTERVAL) \
    .option("checkpointLocation", f"{CHECKPOINT_BASE}/daily_revenue") \
    .queryName("daily_revenue_stream") \
    .start()

print("✓ Streaming query started successfully!")
print(f"Query ID: {daily_query.id}")
print(f"Query Name: {daily_query.name}")
print("\nWaiting for streaming data...")
print("Press Ctrl+C to stop the application\n")

try:
    # Monitor the streaming query status
    while daily_query.isActive:
        status = daily_query.status
        print(f"Query Status: {status['message'] if 'message' in status else 'Running'}")
        
        # Show recent progress
        progress = daily_query.lastProgress
        if progress:
        ...
        exception = daily_query.exception()
        if exception:
    ...
    if daily_query.isActive:
        daily_query.stop()
    spark.stop()
    print("✅ Report 1 streaming application stopped successfully.")
    print(f"Final output location: {OUTPUT_BASE}/daily_revenue")
